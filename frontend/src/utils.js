...
const getRestaurantById = async (restaurantId) => {
  try {
    ...
    const restaurant = Object.values(allRestaurants).find(
...
const getReviewsByUserId = async (userId) => {
  try {
    const response = await axios.get(
      "https://databases-plus-one-b6341ffdbbfd.herokuapp.com/reviews"
    );
...
const updateReview = async (userId, restaurantId, review, rating) => {
  try {
    const response = await axios.put(
      `https://databases-plus-one-b6341ffdbbfd.herokuapp.com/reviews/${userId}/${restaurantId}/${encodeURIComponent(
        review
      )}/${rating}`
    );
...
async function calculateAverageRating(restaurantId) {
  try {
    ...
    const totalRatings = filteredReviews.reduce(
...
async function deleteReview(userId, restaurantId) {
  const url = `https://databases-plus-one-b6341ffdbbfd.herokuapp.com/reviews/delete/${userId}/${restaurantId}`;
